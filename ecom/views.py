...
def search_view(request):
    ...
    products=models.Product.objects.all().filter(Q(price__icontains=query) | Q(name__icontains=query))
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        counter=product_ids.split('|')
        product_count_in_cart=len(set(counter))
...
def update_cart(request):
    for key, value in request.POST.items():
        if key.startswith('quantity-'):
            ...
            quantity = int(value)
            # Giả sử bạn có một hàm để cập nhật số lượng sản phẩm trong giỏ hàng
...
def payment_success_view(request):
    ...
    for product in products:
        models.Orders.objects.get_or_create(customer=customer,product=product,status='Đang chờ',email=email,mobile=mobile,address=address)
